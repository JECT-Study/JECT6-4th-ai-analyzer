...
블로그 글 청킹·임베딩, LLM 분석, 분석 기반 대화, HTML 크롤링 파이프라인을 제공하는
FastAPI 기반 비동기 AI 백엔드.

> - Spring 메인 서버 ↔ 본 서버 통신 계약: [docs/CONTRACT.md](docs/CONTRACT.md)
...
analyzer-server ─┬─▶ Postgres + pgvector  (문서/청크/분석 잡)
...
| 유사도 검색 | `POST /v1/documents/similarity` | 동기 HTTP | — |
...
### 2) 유사도 매칭 — `POST /v1/documents/similarity`
외부 블로그/공고 텍스트로 본인 블로그 글을 검색.

- `use_hyde=true` — 공고/외부 블로그를 LLM으로 1인칭 가상 본문 변환 후 임베딩
  (벡터 매칭 정확도↑)
- `use_hybrid=true` — pgvector + BM25(`tsvector`) RRF 결합 (키워드 매칭↑)
- 둘 다 켜면: HyDE는 임베딩 쿼리에만, BM25는 원본 키워드 사용
  (변환문에서 옅어지는 고유명사 보완)

청크 단위로 검색하되 `DISTINCT ON (document_id)` + 외부 정렬로 **문서당 최고 점수
청크 1개**만 추려 사용자 단위(=문서)로 집계.

### 3) 크롤링 작업 등록 — `POST /v1/crawl/jobs`
URL을 Redis Stream `crawl:jobs`에 등록 → `crawl_worker`가 fetch → HTML 본문 추출
→ 기존 청킹/임베딩 흐름으로 pgvector 저장. 원본 HTML 보관소(MinIO 등)는 사용하지
않는다.
...
### 4) 블로그 글 분석 — `POST /v1/analysis` (동기) 또는 RabbitMQ `blog.analysis`
...
### 5) 분석 기반 대화 — `POST /v1/conversations/messages`
...
- **벡터 인덱스**: HNSW(cosine, m=16, ef_construction=64). 대용량 시 `ef_search`
  튜닝 또는 IVFFlat 전환.
- **Hybrid 검색**: tsvector는 `simple` config. 한국어 정밀도 필요 시 mecab/
  pg_bigm 추가.
...
`tests/unit/`에 10개 모듈, 단위 테스트 23개 (chunker / query rewriter / rate
limiter / embedding cache / html extractor / url security / crawl service /
crawl worker / document service / conversation service). `tests/unit/fakes.py`
의 in-memory Redis/LLM 더블로 외부 의존 없이 service까지 검증.
...
| HyDE 프롬프트 | `app/service/query_rewriter.py` (job/blog 두 종류) |
| 검색 SQL 튜닝 | `app/repository/similarity_repository.py` |
